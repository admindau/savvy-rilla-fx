...
// Dynamic metadata so the <title> starts with the live number
export async function generateMetadata() {
  const res = await fetch(
    'https://fx.savvyrilla.tech/api/v1/latest?base=SXP&symbols=USD',
    { cache: 'no-store' }
  );
  const data = await res.json();
  const usdToSxp =
    data?.rates?.USD ? (1 / Number(data.rates.USD)).toFixed(2) : '—';

  return {
    title: `1 USD = ${usdToSxp} SXP — Today (Black Market) | Savvy Rilla FX`,
    description: `Live USD to SXP black-market rate: 1 USD = ${usdToSxp} SXP (today). Full history and daily trends from Savvy Rilla FX.`,
    alternates: { canonical: 'https://fx.savvyrilla.tech/usd-to-sxp' },
    openGraph: {
      title: `1 USD = ${usdToSxp} SXP — Black Market`,
      description:
        'Live USD to SXP black-market exchange rate with full historical data.',
    ...
    twitter: {
      ...
      title: `1 USD = ${usdToSxp} SXP — Black Market`,
      description:
        'Live USD to SXP black-market exchange rate from Savvy Rilla FX.',
      images: ['/logo.png'],
    },
  };
}
...
export default async function Page() {
  ...
  // latest SXP→USD (invert to show USD→SXP)
  ...
  const usdToSxp = latest?.rate
    ? Number((1 / Number(latest.rate)).toFixed(4))
    : null;

  const all =
    (hist ?? []).map((r: Row) => ({
      date: r.rate_date,
      value: Number((1 / Number(r.rate)).toFixed(4)),
    })) ?? [];
  const spark = all.slice(-180).map((p) => p.value);

  return (
    <main className="min-h-dvh p-8 text-white bg-black">
      {/* Front-load number in H1 for SEO snippet */}
      <h1 className="text-3xl font-bold">
        1 USD = {usdToSxp?.toLocaleString()} SXP — Today (Black Market)
      </h1>
        ...
        <>
          ...
          <script
            ...
            dangerouslySetInnerHTML={{
              __html: JSON.stringify(exchangeRateSchemaSxp(usdToSxp, date)),
            }}
          />
          ...
          <p className="mt-8 opacity-80">
            Official rate?{' '}
            <a className="underline" href="/usd-to-ssp">
              USD → SSP
            </a>
          </p>
        </>
