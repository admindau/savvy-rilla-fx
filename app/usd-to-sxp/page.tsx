...
export async function generateMetadata() {
  try {
    ...
    const title = rate
      ? `1 USD = ${rate} SXP — Today (Black Market) | Savvy Rilla FX`
      : 'USD to SXP — Today (Black Market) | Savvy Rilla FX';

    return {
      title,
      description: rate
        ? `Live USD to SXP black-market rate: 1 USD = ${rate} SXP (today).`
        : 'Live USD to SXP black-market rate and full history.',
      alternates: { canonical: (process.env.NEXT_PUBLIC_SITE_URL ?? 'https://fx.savvyrilla.tech') + '/usd-to-sxp' },
      openGraph: { title, url: (process.env.NEXT_PUBLIC_SITE_URL ?? 'https://fx.savvyrilla.tech') + '/usd-to-sxp', images: ['/logo.png'] },
      twitter: { card: 'summary_large_image', title, images: ['/logo.png'] },
    };
  } catch {
    return {
      title: 'USD to SXP — Today (Black Market) | Savvy Rilla FX',
      description: 'Live USD to SXP black-market rate and full history.',
...
export default async function Page() {
  ...
  return (
    <main className="container">
      <div className="h1">1 USD = {usdToSxp?.toLocaleString()} SXP — Today (Black Market)</div>
      <div className="muted">Black-market rate on {date}</div>
    ...
    <section className="mt-8">
          ...
          <p className="muted" style={{ marginTop: 20 }}>Official rate? <a href="/usd-to-ssp">USD → SSP</a></p>
        </>
      ) : (
