...
function ldFaq(usdToSsp: number) {
  return {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
    mainEntity: [
      { '@type': 'Question', name: 'How much is $1 in SSP?', acceptedAnswer: { '@type': 'Answer', text: `1 USD equals ${usdToSsp.toLocaleString()} SSP (official).` } },
      { '@type': 'Question', name: 'How much is $10 in SSP?', acceptedAnswer: { '@type': 'Answer', text: `10 USD equals ${(usdToSsp*10).toLocaleString()} SSP (official).` } },
      { '@type': 'Question', name: 'How much is $100 in SSP?', acceptedAnswer: { '@type': 'Answer', text: `100 USD equals ${(usdToSsp*100).toLocaleString()} SSP (official).` } },
    ],
  }
}
...
export default async function Page() {
  ...
  const spark = all.slice(-180).map(p=>p.usdToSsp)
    ...
    <main className="min-h-dvh p-8 text-white bg-black">
      {usdToSsp && (
        <>
          ...
          <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(ldFaq(usdToSsp)) }} />
        </>
      )}
        ...
        <>
          ...
          <div className="mt-6 grid gap-3 w-full max-w-xl">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="bg-zinc-900/60 rounded-xl p-3">
                ...
                <output id="out" className="block text-2xl">{usdToSsp.toLocaleString()}</output>
                <div className="text-xs opacity-70 mt-1">South Sudanese Pound (SSP)</div>
              </div>
            </div>
            <script dangerouslySetInnerHTML={{__html:`
              const a=document.getElementById('amt'), o=document.getElementById('out'), r=${usdToSsp};
          ...
          <div className="mt-6">
            <table className="w-full text-sm border-separate border-spacing-y-1">
              <thead className="opacity-70"><tr><th className="text-left">Date</th><th className="text-right">1 USD in SSP</th></tr></thead>
              <tbody id="rows">
                ${[...all].reverse().slice(0,14).map(r =>
                  `<tr><td>${r.date}</td><td class="text-right">${r.usdToSsp.toLocaleString()}</td></tr>`
                ).join('')}
              </tbody>
            </table>
            ...
            <script dangerouslySetInnerHTML={{__html:`
              document.getElementById('showAll')?.addEventListener('click',()=>{
                ...
                rows.innerHTML=data.map(r=>\`<tr><td>\${r.date}</td><td class="text-right">\${Number(r.usdToSsp).toLocaleString()}</td></tr>\`).join('');
              });
            `}}/>
          </div>
